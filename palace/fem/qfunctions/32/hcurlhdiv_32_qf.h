...
CEED_QFUNCTION(f_apply_hcurlhdiv_32)(void *__restrict ctx, CeedInt Q,
...
CEED_QFUNCTION(f_apply_hdivhcurl_32)(void *__restrict ctx, CeedInt Q,
                                     const CeedScalar *const *in, CeedScalar *const *out)
{
  const CeedScalar *attr = in[0], *wdetJ = in[0] + Q, *adjJt = in[0] + 2 * Q, *u = in[1];
  CeedScalar *v = out[0];
...
#endif  // PALACE_LIBCEED_HCURL_HDIV_32_QF_H

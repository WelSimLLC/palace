...
{
  using fmt::format;
  domain_E = TableWithCSVFile(fs::path(post_dir/"domain-E.csv").string(), reload_table);
...
void PostOperatorCSV<solver_t>::InitializeSurfaceF(const SurfacePostOperator &surf_post_op)
{
  ...
  using fmt::format;
  surface_F = TableWithCSVFile(fs::path(post_dir / "surface-F.csv").string(), reload_table);
...
void PostOperatorCSV<solver_t>::InitializeSurfaceQ(const SurfacePostOperator &surf_post_op)
{
  ...
  using fmt::format;
  surface_Q = TableWithCSVFile(fs::path(post_dir / "surface-Q.csv").string(), reload_table);
    ...
    -> std::enable_if_t<U == ProblemType::DRIVEN || U == ProblemType::EIGENMODE, void>
{
  ...
  farfield_E = TableWithCSVFile(fs::path(post_dir / "farfield-rE.csv").string());
...
void PostOperatorCSV<solver_t>::InitializeProbeE(const InterpolationOperator &interp_op)
{
  ...
  using fmt::format;
  probe_E = TableWithCSVFile(fs::path(post_dir / "probe-E.csv").string(), reload_table);
...
void PostOperatorCSV<solver_t>::InitializeProbeB(const InterpolationOperator &interp_op)
{
  ...
  using fmt::format;
  probe_B = TableWithCSVFile(fs::path(post_dir / "probe-B.csv").string(), reload_table);
  Table t;  // Define table locally first due to potential reload.
  auto v_dim = interp_op.GetVDim();
  int scale_col = (HasComplexGridFunction<solver_t>() ? 2 : 1) * v_dim;
  auto nr_expected_measurement_cols =
    ...
    -> std::enable_if_t<U == ProblemType::DRIVEN || U == ProblemType::TRANSIENT, void>
{
  ...
  using fmt::format;
  surface_I = TableWithCSVFile(fs::path(post_dir / "surface-I.csv").string(), reload_table);
                        ...
                        void>
{
  ...
  port_V = TableWithCSVFile(fs::path(post_dir / "port-V.csv").string(), reload_table);
  port_I = TableWithCSVFile(fs::path(post_dir / "port-I.csv").string(), reload_table);
    ...
    -> std::enable_if_t<U == ProblemType::DRIVEN, void>
{
  ...
  using fmt::format;
  port_S = TableWithCSVFile(fs::path(post_dir / "port-S.csv").string(), reload_table);
...
{
  using fmt::format;
  eig = TableWithCSVFile(fs::path(post_dir / "eig.csv").string());
    ...
    -> std::enable_if_t<U == ProblemType::EIGENMODE, void>
{
  ...
  using fmt::format;
  port_EPR = TableWithCSVFile(fs::path(post_dir / "port-EPR.csv").string());
    ...
    -> std::enable_if_t<U == ProblemType::EIGENMODE, void>
{
  ...
  using fmt::format;
  port_Q = TableWithCSVFile(fs::path(post_dir / "port-Q.csv").string());
    ...
    bool is_root, const ErrorIndicator::SummaryStatistics &indicator_stats)
{
  ...
  TableWithCSVFile error_indicator(fs::path(post_dir / "error-indicators.csv").string());
